...
public class TestDateTimeInput {
	...
	public void testAriaLabel() {
		Input<Date> input = Input.dateTime().ariaLabel("Date-time input").build();
		assertNull(ComponentTestUtils.getElementAttribute(input.getComponent(), "aria-label"));

		input = Input.dateTime().ariaLabelledBy("date-time-input-label").build();
		assertEquals("date-time-input-label",
				ComponentTestUtils.getElementAttribute(input.getComponent(), "aria-labelledby"));

		input = Input.dateTime().ariaLabel(Localizable.builder().message("Localized date-time").build()).build();
		assertNull(ComponentTestUtils.getElementAttribute(input.getComponent(), "aria-label"));
	}
