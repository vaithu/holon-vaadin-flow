...
//		assertTrue(input.getComponent() instanceof HasPrefix);
	...
	public void testAdapters() {
		...
		Input<Double> input = Input.number(Double.class).withAdapter(TestAdapter.class, i -> TestAdapter.create(i, 789))
				.build();
		...
		input = Input.number(Double.class).validatable().withAdapter(TestAdapter.class, i -> TestAdapter.create(i, 789))
				.build();
