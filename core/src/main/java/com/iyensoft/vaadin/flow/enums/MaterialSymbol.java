...
public final class MaterialSymbol {

    public static MaterialSymbol _10K,
    _10MP,
    _11MP,
    _123,
    _12MP,
    ...
    private static final Map<String, MaterialSymbol> BY_NAME = new LinkedHashMap<>();
    private static final MaterialSymbol[] VALUES;

    static {
        try {
            for (Field field : MaterialSymbol.class.getDeclaredFields()) {
                if (Modifier.isStatic(field.getModifiers()) && field.getType() == MaterialSymbol.class) {
                    MaterialSymbol symbol = new MaterialSymbol(field.getName());
                    field.set(null, symbol);
                    BY_NAME.put(field.getName(), symbol);
                }
            }
        } catch (IllegalAccessException e) {
            throw new ExceptionInInitializerError(e);
        }
        List<MaterialSymbol> values = new ArrayList<>(BY_NAME.values());
        VALUES = values.toArray(new MaterialSymbol[0]);
    }

    private final String name;

    private MaterialSymbol(String name) {
        this.name = name;
    }
    ...
    public static MaterialSymbol[] values() {
        return VALUES.clone();
    }

    public static List<MaterialSymbol> valuesList() {
        return Collections.unmodifiableList(List.of(VALUES));
    }

    public static MaterialSymbol valueOf(String name) {
        MaterialSymbol value = BY_NAME.get(name);
        if (value == null) {
            throw new IllegalArgumentException("No enum constant " + MaterialSymbol.class.getName() + "." + name);
        }
        return value;
    }
