...
//    private final MobileColumn mobileColumn  = new MobileColumn();
    ...
    private void displayIndexColumnAsFirst(Column<T> rowIndex) {
    ...
    public void removeColumnByKey(String columnKey) {
        getGrid().removeColumnByKey(columnKey);
    }
    ...
    private void reOrderColumnsForMobile() {
        List<Column<T>> columns = new ArrayList<>(getAllColumns());
//        getGrid().removeAllColumns();
    ...
    public void setMobileColumn(Renderer<T> renderer) {
        mobileColumn = getGrid().addColumn(renderer).setKey(MOBILE_COLUMN_KEY);
        reOrderColumnsForMobile();
    }

    @Override
    public void setMobileColumn(ValueProvider<T, Component> component) {
        mobileColumn = getGrid().addComponentColumn(component).setKey(MOBILE_COLUMN_KEY);
        reOrderColumnsForMobile();
    }
    ...
    public void showMobileColumn(boolean mobile) {
        ...
        if (mobileColumn != null) {
            ...
            for (int i = 1; i < allColumns.size(); i++) {
                allColumns.get(i).setVisible(!mobile);
            }
        }
    }
    ...
    public void addThemeVariants(GridVariant... variants) {
        getGrid().addThemeVariants(variants);
    }
...
*/
    ...
    @Override
    public void setFrozenMultiSelectCheckBoxColumn(boolean freeze) {
        ((GridMultiSelectionModel<?>) getGrid()
                .setSelectionMode(Grid.SelectionMode.MULTI))
                .setSelectionColumnFrozen(freeze);
    }


    @Override
    public void setScrollUsingUpDownKeys() {
    ...
    public void restoreAllColumns() {
        getGrid().getColumns().forEach(tColumn -> tColumn.setVisible(true));
    }
    ...
    /*protected void setAutoCreateColumns(boolean autoCreateColumns) {
        this.autoCreateColumns = autoCreateColumns;
        if (!this.autoCreateColumns) {
            getGrid().removeAllColumns();
            this.visibleColumns = Collections.emptyList();
            log.info("Removed all columns because autoCreateColumns is {}", autoCreateColumns);
        } else {
            this.visibleColumns.forEach(p -> addPropertyColumn(p));
        }
    }*/
    ...
    private GridLazyDataView<T> getGridLazyDataView() {
      ...
      /*  DataProvider<Person, ?> dataProvider = grid.getDataProvider();
        ...
        if (dataProvider instanceof BackEndDataProvider) {
            System.out.println("The grid is using a BackEndDataProvider.");
        } else if (dataProvider instanceof AbstractDataProvider) {
            System.out.println("The grid is using an AbstractDataProvider.");
        } else {
            System.out.println("The grid is using a different type of DataProvider.");
        }*/
    }
...
/*
        @Override
        public C mobileColumn(Property<?> property, Renderer<T> renderer) {
            instance.setMobileColumn(renderer);
            return getConfigurator();
        }

        @Override
        public C mobileHeader(String header) {
            instance.setMobileHeader(header);
            return getConfigurator();
        }

        @Override
        public C showMobileColumn(boolean show) {
            instance.showMobileColumn(show);
            return getConfigurator();
        }*/

        @Override
        public C mobileColumn(P property, Renderer<T> renderer) {
            return renderer(property, renderer);
        }

        @Override
        public C freezeMultiSelectCheckBoxColumn(boolean freeze) {
            instance.setFrozenMultiSelectCheckBoxColumn(freeze);
            return getConfigurator();
        }

        @Override
        public C scrollUsingUpDownKeys() {
            instance.setScrollUsingUpDownKeys();
            return getConfigurator();
        }
         ...
         */
       /* @Override
        public C autoCreateColumns(boolean autoCreateColumns) {
            instance.setAutoCreateColumns(autoCreateColumns);
            return getConfigurator();
        }*/
       ...
       /* @Override
        public C withIndexColumn() {
            instance.addIndexColumn();
            return getConfigurator();
        }

        @Override
        public C withIndexColumn(P property) {
            instance.addIndexColumn(property);
            return getConfigurator();
        }*/
         ...
         */
        @Override
        public C frozenAtEnd(P property, boolean frozen) {
            instance.getColumnConfiguration(property).setFrozenAtEnd(frozen);
            return getConfigurator();
        }

        @Override
        public C tooltipGenerator(P property, SerializableFunction<T, String> tooltipGenerator) {
            instance.getColumnConfiguration(property).setTooltipGenerator(tooltipGenerator);
            return getConfigurator();
        }
        ...
        @Override
        public C partNameGenerator(SerializableFunction<T, String> partNameGenerator) {
            if (partNameGenerator != null) {
                instance.getGrid().setPartNameGenerator(partNameGenerator);
            }
            return getConfigurator();
        }
    ...
    private static class ItemListingContextMenuItemBuilder<T, P, B extends ContextMenuConfigurator<ItemEventListener<GridMenuItem<T>, T, ItemListingItemEvent<GridMenuItem<T>, T, P>>, GridContextMenu<T>, GridMenuItem<T>, GridSubMenu<T>, B>>
         ...
         */
        @Override
        public MenuItemBuilder<ItemEventListener<GridMenuItem<T>, T, ItemListingItemEvent<GridMenuItem<T>, T, P>>, GridContextMenu<T>, GridMenuItem<T>, GridSubMenu<T>, B> keepOpen(boolean keepOpen) {
