 ...
 * @since 5.2.0
 */
public interface HasBeanDatastoreFilterableDataProviderConfigurator<T, F, D extends DatastoreDataProviderConfigurator<T, D>, C extends HasBeanDatastoreFilterableDataProviderConfigurator<T, F, D, C>>
		extends HasFilterableDataProviderConfigurator<T, F, C> {
	...
	default D dataSource(Datastore datastore, DataTarget<?> target, Function<PropertyBox, T> itemConverter,
		...
		return dataSource(datastore, target, itemConverter, filterConverter, PropertySet.of(properties));
	}
	 ...
	 * @see DatastoreDataProviderConfigurator
	 */
	D dataSource(Datastore datastore, DataTarget<?> target, Function<F, QueryFilter> filterConverter);
