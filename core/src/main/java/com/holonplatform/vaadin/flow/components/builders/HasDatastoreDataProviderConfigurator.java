...
/**
 * {@link HasDataProviderConfigurator} with {@link Datastore} configuration support.
 * 
 * @param <ITEM> Data item type
 * @param <C> Concrete configurator type
 *
 * @since 5.2.0
 */
public interface HasDatastoreDataProviderConfigurator<ITEM, D extends DatastoreDataProviderConfigurator<ITEM, D>, C extends HasDatastoreDataProviderConfigurator<ITEM, D, C>>
		extends HasDataProviderConfigurator<ITEM, C> {
	...
	<P extends Property> D dataSource(Datastore datastore, DataTarget<?> target,
			Function<PropertyBox, ITEM> itemConverter, Iterable<P> properties);
	 ...
	 * @see DatastoreDataProviderConfigurator
	 */
	default D dataSource(Datastore datastore, DataTarget<?> target, Function<PropertyBox, ITEM> itemConverter,
			Property<?>... properties) {
	 ...
	 * @see DatastoreDataProviderConfigurator
	 */
	D dataSource(Datastore datastore, DataTarget<?> target, Class<ITEM> beanType);
