...
import com.holonplatform.vaadin.flow.internal.data.DefaultDatastoreLazyDataProvider;
...
/**
 * A {@link BackEndDataProvider} which uses a {@link Datastore} for fetching data from
 * a backend.
 * 
 * @param <T> Data type
 ...
 * @since 5.5.4
 */
public interface DatastoreLazyDataProvider<T, F> extends BackEndDataProvider<T, F> {
	...
	/**
	 * Create a new {@link DatastoreLazyDataProvider} which uses {@link PropertyBox}
	 ...
	 * @return A new {@link DatastoreLazyDataProvider}
	 */
	static DatastoreLazyDataProvider<PropertyBox, QueryFilter> create(Datastore datastore, DataTarget<?> target,
	 ...
	 * @return A new {@link DatastoreLazyDataProvider}
	 */
	static <F> DatastoreLazyDataProvider<PropertyBox, F> create(Datastore datastore, DataTarget<?> target,
	...
	/**
	 * Create a new {@link DatastoreLazyDataProvider} which uses given
	 ...
	 * @return A new {@link DatastoreLazyDataProvider}
	 */
	static <T> DatastoreLazyDataProvider<T, QueryFilter> create(Datastore datastore, DataTarget<?> target,
                                                                Class<T> beanType) {
	 ...
	 * @return A new {@link DatastoreLazyDataProvider}
	 */
	static <T, F> DatastoreLazyDataProvider<T, F> create(Datastore datastore, DataTarget<?> target, Class<T> beanType,
                                                         Function<F, QueryFilter> filterConverter) {
		return builder(datastore, target, beanType, filterConverter).build();
	}

	/**
	 * Create a new {@link DatastoreLazyDataProvider}.
	 * @param <T>             Data type
	 * @param <F>             Query filter type
	 * @param datastore       The {@link Datastore} to use (not null)
	 * @param target          The {@link DataTarget} to use as query target (not
	 ...
	 * @return A new {@link DatastoreLazyDataProvider}
	 */
	static <T, F> DatastoreLazyDataProvider<T, F> create(Datastore datastore, DataTarget<?> target,
	...
	/**
	 * Get a builder to create and configure a new {@link DatastoreLazyDataProvider}
	 * which uses {@link PropertyBox} items type and {@link QueryFilter} type data
	 * provider filters.
	 * @param datastore   The {@link Datastore} to use (not null)
	 * @param target      The {@link DataTarget} to use as query target (not null)
	 * @param propertySet The property set to use as query projection (not null)
	 * @return a new {@link DatastoreLazyDataProvider} builder
	 */
	static PropertyBoxItemBuilder<QueryFilter> builder(Datastore datastore, DataTarget<?> target,
			PropertySet<?> propertySet) {
		return new DefaultDatastoreLazyDataProvider.DefaultPropertyBoxItemBuilder<>(datastore, target, propertySet,
	...
	static PropertyBoxItemBuilder<QueryFilter> builder(Datastore datastore, DataTarget<?> target,
			Property<?>... properties) {
		return new DefaultDatastoreLazyDataProvider.DefaultPropertyBoxItemBuilder<>(datastore, target,
	...
	static <T> Builder<T, QueryFilter> builder(Datastore datastore, DataTarget<?> target, Class<T> beanType) {
		...
		return new DefaultDatastoreLazyDataProvider.DefaultBuilder<>(datastore, target, beanPropertySet,
	...
	static <T, F> Builder<T, F> builder(Datastore datastore, DataTarget<?> target, PropertySet<?> propertySet,
		...
		return new DefaultDatastoreLazyDataProvider.DefaultBuilder<>(datastore, target, propertySet, itemConverter,
				filterConverter);
	}
	...
	/**
	 * {@link DatastoreLazyDataProvider} builder.
	 * 
	 * @param <T> Data type
	 * @param <F> Query filter type
	 */
	public interface Builder<T, F> {
		...
		/**
		 * Build the {@link DatastoreLazyDataProvider}.
		 * @return The {@link DatastoreLazyDataProvider} instance
		 */
		DatastoreLazyDataProvider<T, F> build();

	}

	/**
	 * A {@link DatastoreLazyDataProvider} with {@link PropertyBox} items type builder.
	 * 
	 * @param <F> Query filter type
	 */
