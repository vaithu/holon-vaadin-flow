 ...
 * @since 5.2.0
 */
public class DefaultItemListingHeaderRow<P> implements EditableItemListingRow<P> {

	private final HeaderRow row;
	private final Function<P, Column<?>> propertyColumnProvider;

	public DefaultItemListingHeaderRow(HeaderRow row, Function<P, Column<?>> propertyColumnProvider) {
		super();
	...
	public Optional<ItemListingCell> getCell(P property) {
		...
		if (column != null) {
			try {
				return Optional.of(new DefaultItemListingHeaderCell(row.getCell(column)));
