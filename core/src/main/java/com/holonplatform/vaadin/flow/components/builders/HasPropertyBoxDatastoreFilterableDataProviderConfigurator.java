...
/**
 * {@link HasFilterableDataProviderConfigurator} with {@link Datastore} configuration support and using
 * {@link PropertyBox} type items.
 * 
 * @param <F> Filter type
 ...
 * @since 5.2.0
 */
public interface HasPropertyBoxDatastoreFilterableDataProviderConfigurator<F, D extends DatastoreDataProviderConfigurator<PropertyBox, D>, C extends HasPropertyBoxDatastoreFilterableDataProviderConfigurator<F, D, C>>
		extends HasFilterableDataProviderConfigurator<PropertyBox, F, C> {
	...
	<P extends Property> D dataSource(Datastore datastore, DataTarget<?> target,
	 ...
	 * @see DatastoreDataProviderConfigurator
	 */
	default D dataSource(Datastore datastore, DataTarget<?> target, Function<F, QueryFilter> filterConverter,
			Property<?>... properties) {
		return dataSource(datastore, target, filterConverter, PropertySet.of(properties));
	}
