...
/**
 * Default header {@link ItemListingSection} implementation.
 * 
 * @param <P> Item property type
 * 
 * @since 5.2.0
 */
public class DefaultItemListingHeaderSection<P> implements EditableItemListingSection<P> {

	private final Grid<?> grid;
	private final Function<P, Column<?>> propertyColumnProvider;

	public DefaultItemListingHeaderSection(Grid<?> grid, Function<P, Column<?>> propertyColumnProvider) {
		super();
	...
	public EditableItemListingRow<P> appendRow() {
		return new DefaultItemListingHeaderRow<>(grid.appendHeaderRow(), propertyColumnProvider);
	}
	...
	public EditableItemListingRow<P> prependRow() {
		return new DefaultItemListingHeaderRow<>(grid.prependHeaderRow(), propertyColumnProvider);
	}
