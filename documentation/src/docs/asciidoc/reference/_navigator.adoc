...
When the `holon-vaadin-flow-navigator` artifact is in classpath, the `jakarta.annotation.security.RolesAllowed` and `jakarta.annotation.security.PermitAll` annotations can be used on routing target classes to perform a *role based authorization access control* on routing targets.
...
When the `jakarta.annotation.security.RolesAllowed` annotation is used, *any of the specified role name* must be granted to the current authenticated user in order to allow the route access.
