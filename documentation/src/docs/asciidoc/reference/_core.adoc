...
New reusable UI components in this module should follow a small, consistent set of UX rules:

* Prefer semantic components with a single clear role, such as header, footer, panel, dialog, or listing.
* Prefer slot-based composition for structure: header, content, actions, meta, footer, and similar regions.
* Keep the Java API declarative and small: create, configure, and compose, but do not hide layout logic in complex builders.
* Keep all visual styling in external CSS files, and use class names or theme hooks instead of inline style logic.
* Provide accessible defaults: proper landmarks, meaningful labels, predictable tab order, and keyboard-friendly interactions.
* Make responsive behavior explicit and conservative: stack on small screens, expand progressively on larger screens, and avoid layout surprises.
* Use background, border, and emphasis as semantic states, not as ad-hoc styling knobs.
* Prefer a thin builder/configurator pair for reusable components, with the component itself remaining the source of truth.

When a new component does not clearly improve UX or reduce complexity, it should not be added as a separate standard component.
...
<6> Set whether multiple column sorting is enabled on the client-side
<7> Set the column auto-width mode for the `ID` property
<8> Set the auto-width mode for all the listing columns
<9> Add a listener for listing columns resize events
<10> Add a listener for listing columns reordering events
...
WARNING: Since version 5.2.7, the link:{apidir}/com/holonplatform/vaadin/device/DeviceInfo.html[DeviceInfo^] API is deprecated: use `com.vaadin.flow.server.WebBrowser` instead (it can be obtained from `VaadinSession`). 
